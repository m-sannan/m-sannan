<img src="./assets/banner.svg" width="100%" alt="Sannan — World 1-1 Product Manager, World 1-2 Designer" />

## Hello

I’m **Sannan**. Product manager at [Alyve Health](https://www.alyve.health/). I also (used to) design packaging on earlier.

Vibe coding products and questing my CS degree.

4+ years in product — digital health, payments, partner integrations, and gamification.

[![LinkedIn](https://img.shields.io/badge/linkedin-%230077B5.svg?style=for-the-badge&logo=LinkedIn&logoColor=white)](https://www.linkedin.com/in/mohammedsannan/)
[![Website](https://img.shields.io/badge/website-111827?style=for-the-badge&logo=vercel&logoColor=white)](https://msannan.vercel.app)

![](https://visitcount.itsvg.in/api?id=m-sannan&label=Profile%20views&color=12&icon=5&pretty=true)

## Work

**Alyve Health — Product Manager**  
Lead teleconsult, pharmacy, flex, and partly alyve pay. 

**CustomerGlu — APM → product owner**  
Led the end-user gamification pod at 40M+ scale — the UI players see and play. Worked on campaigns from Rupeek, Nykaa and other FMCG brands.

**buildd / Flexiple — consultant**  
Quick stint here, Led over 20+ projects incorporating gamification elements into user experience and content feed enhancements, 46% increase in MAU.

## A few things I made

**[OthersPe](https://otherspe.vercel.app)** — live  
Pay a UPI QR for someone else’s behalf. Friend opens a pay page; picks CRED or any UPI app. Money never touches OthersPe. Similar to UPI Circle but we tried making this in 2023.

**[Staycheck](https://github.com/m-sannan/staycheck)** — Chrome extension  
Second opinion on hotel listings across Booking, Agoda, and Expedia. Check Google Map reviews, before you book.

**[EchoLog](https://github.com/m-sannan/echolog)** — Chrome extension  
Captures network requests on domains you choose. For bugs that only appear when nobody is watching.

**[Cloneify](https://cloneify.framer.ai/)** — Figma plugin  
Copy styles across objects in one click. #3 Product of the Day on Product Hunt. 1,800+ users.

**[Sheeto](https://github.com/m-sannan/Sheeto)** — Mini software  
Opens local Excel/CSV in Google Sheets in one click.

**Also:** [SnapNotes](https://github.com/m-sannan/snapnotes) · [backfill](https://github.com/m-sannan/backfill-extension) · [ps5-shelf](https://github.com/m-sannan/ps5-shelf)

## Stats

![GitHub stats](https://github-stats-extended.vercel.app/api?username=m-sannan&show_icons=true&theme=github_dark&bg_color=00000000&hide_border=true&hide=issues,contribs)
![Top langs](https://github-stats-extended.vercel.app/api/top-langs/?username=m-sannan&layout=compact&theme=github_dark&bg_color=00000000&hide_border=true)
